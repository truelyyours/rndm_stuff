...
case "$TERM" in
xterm*|rxvt*|tmux*)
    if [ -n "$(ps -ef | grep 'openvpn .*\.ovpn'| grep -v 'grep')" ]; then
      VPN=$(ps -ef | grep 'openvpn .*\.ovpn'| grep -v 'grep' |tail -1| rev| awk '{print $1}'|rev |sed 's/\..*$//g')
...
source ~/py13.7venv/venv/bin/activate
